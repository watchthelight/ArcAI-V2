...
inline float lstm_train_batch(LSTM& M, const uint8_t* batch /*size: BATCH*SEQ_LEN*/) {
    ...
    // temps
    float* Hprev = (float*)arc_aligned_malloc((size_t)B*H*sizeof(float));
    float* Cprev = (float*)arc_aligned_malloc((size_t)B*H*sizeof(float));
    float* Hcur  = (float*)arc_aligned_malloc((size_t)B*H*sizeof(float));
    float* Ccur  = (float*)arc_aligned_malloc((size_t)B*H*sizeof(float));
    float* Z     = (float*)arc_aligned_malloc((size_t)B*V*sizeof(float));
    float* dZ    = (float*)arc_aligned_malloc((size_t)B*V*sizeof(float));
    ...
    arc_aligned_free(Hprev); arc_aligned_free(Cprev);
    arc_aligned_free(Hcur); arc_aligned_free(Ccur);
    arc_aligned_free(Z);     arc_aligned_free(dZ);
