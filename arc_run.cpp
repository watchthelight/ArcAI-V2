...
// arc_generate.h brings sample helpers and ensures rnn_forward is visible via arc_kernels.h
#include "arc_generate.h"
...
static Args parse_args(int argc, char** argv){
    Args a;
    if (argc < 2){
        std::fprintf(stderr, "usage: %s checkpoint.bin [--len N] [--temp T] [--topk K] [--seed STR]\n", argv[0]);
        std::exit(1);
    }
    a.checkpoint = argv[1];
    for (int i=2;i<argc;i++){
        if      (std::strcmp(argv[i],"--len")==0  && i+1<argc) a.len  = std::stoi(argv[++i]);
        else if (std::strcmp(argv[i],"--temp")==0 && i+1<argc) a.temp = std::stof(argv[++i]);
        else if (std::strcmp(argv[i],"--topk")==0 && i+1<argc) a.topk = std::stoi(argv[++i]);
        else if (std::strcmp(argv[i],"--seed")==0 && i+1<argc) a.seed = argv[++i];
    }
    return a;
}
...
// Same binary format as in arc_train.cpp
