...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::fprintf(stderr, "usage: %s dataset.bin\n", argv[0]);
        return 1;
    }
    ...
    std::printf("Training: BATCH=%d SEQ_LEN=%d HIDDEN=%d VOCAB=%d steps=10000\n",
        BATCH, SEQ_LEN, HIDDEN, VOCAB_SIZE);
