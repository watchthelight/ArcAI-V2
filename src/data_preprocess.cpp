...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: " << argv[0] << " <input_text_file> [output_bin] [vocab_json]" << std::endl;
        std::cerr << "  input_text_file: Path to text file to preprocess" << std::endl;
        std::cerr << "  output_bin: Output binary file (default: dataset.bin)" << std::endl;
        std::cerr << "  vocab_json: Output vocabulary JSON (default: vocab.json)" << std::endl;
        return 1;
    }

    std::string input_file = argv[1];
    std::string output_bin = (argc >= 3) ? argv[2] : "dataset.bin";
    std::string vocab_json = (argc >= 4) ? argv[3] : "vocab.json";

    try {
        preprocess_data(input_file, output_bin, vocab_json);
